...
* Benchmark window:
  * Before the benchmark starts:
    * [x] Warn about benchmark discard current document
    * [x] Allow disabling syntax highlight
    * [x] Choose between 3 different benchmarks
  * Do this automatically on the benchmark start:
    * [x] Disable user interaction
    * [ ] Reserve string space for gradual benchmark (can't do that with QPlainTextEdit?)
    * [x] Change text/canvas layout ratio
    * [x] Maximize the window
    * [x] Move the bench-window window top-left of the parent
  * During the benchmark:
    * [x] Set bench-window unclosable when bench running and show undetermined progress bar
    * [x] Don't mark the document as dirty
    * [x] Show FPS, total nodes, app ram usage, system cpu usage; win+linux
    * [x] Record shown data with timestamp
  * Post:
    * [x] Export to CSV
...
* (visual) Syntax highlight:
  ...
  * On Linux the keywords are not bold (Windows OK)
* (visual) Icons stay black after changing to dark mode (should switch to white as the resto of the font)
* (minor) Canvas reset scrolling after app startup not working as expected
* (annoying) On Windows (Linux OK), when dragging canvas via RMB and then releasing RMB outside of canvas on some other widget (e.g. textedit), a context menu for that widget appears, as if we right-clicked on that widget intentionally
* (different to IMGUI) Settings in preferences window, that can also be changed from toolbar/menubar, won't get updated immediatelly when changed from those places (they get updated after reopening the preferences window). Preferences window also has an 'apply all' button. This could be improved, but would require additional logic.
* (sad) Turning on syntax highlight in preferences window and hitting apply will mark the document as dirty.
